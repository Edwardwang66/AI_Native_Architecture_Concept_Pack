...
仓库目前包含**已经批准的设计**与**尚未验证的 Skill 草案**，不包含生产实现，也没有任何宿主已经取得行为验证资格。

| 轨道 | 已批准方向 | 当前状态 |
|---|---|---|
| Part I — AI Native Architect Skill | 遵循 Agent Skills 格式的可移植核心＋独立宿主适配层；OpenClaw 为首个行为验证目标 | 设计已批准；实现和验证尚未开始 |
| Part II — Capability Control Plane | Architecture-to-Policy 编译器＋独立保障层；MVP 只读且不进入运行时数据路径 | 设计已批准；实现和市场验证尚未开始 |
...
4. 将两份 RFC 与原始中文企划、现有 Skill 草案对照。
5. 通过公开 issue 提交缺失证据、反例、安全边界、兼容性或实现风险。
